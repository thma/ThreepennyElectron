...
Now we use `processCommand` to generate calculator state transition function based on each comannd in the stream. 
...
In the next step we define a `Behaviour` based on the `commands` stream of state transitions.
